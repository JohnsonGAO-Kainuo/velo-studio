...
electron.contextBridge.exposeInMainWorld("electronAPI", {
  ...
  hideMainWindow: () => {
    return electron.ipcRenderer.invoke("hide-main-window");
  },
  showMainWindow: () => {
    return electron.ipcRenderer.invoke("show-main-window");
  },
  minimizeMainWindow: () => {
    return electron.ipcRenderer.invoke("minimize-main-window");
  },
  restoreMainWindow: () => {
    return electron.ipcRenderer.invoke("restore-main-window");
  },
  // Cursor tracking for auto-zoom
  startCursorTracking: () => {
    return electron.ipcRenderer.invoke("start-cursor-tracking");
  },
  stopCursorTracking: () => {
    return electron.ipcRenderer.invoke("stop-cursor-tracking");
  },
  recordCursorClick: (button) => {
    return electron.ipcRenderer.invoke("record-cursor-click", button);
  },
  saveCursorData: (videoPath, cursorData) => {
    return electron.ipcRenderer.invoke("save-cursor-data", videoPath, cursorData);
  },
