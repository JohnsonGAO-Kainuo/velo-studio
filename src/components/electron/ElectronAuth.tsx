          ...
          if (data.code) {
            ...
            if (exchangeError) {
              ...
              if (sessionData.session?.user) {
                ...
                const accessOk = await ensureAccessAndNotifyMain(sessionData.session.user.id);
                if (!accessOk) {
                  console.warn('[ElectronAuth] Code exchange OK but no access — profile may not exist yet');
                  setError('Unable to verify your account. Please restart the app and try again.');
                }
              } else {
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-[#f2f2f2] p-6">
        <div className="w-full max-w-sm text-center">
          ...
          <h2 className="text-xl font-bold text-[#202020] mb-2">Complete Sign-In</h2>
          <p className="text-sm text-[#5c5c5c] mb-6">
            Sign in with Google in your browser, then click <strong>"Open Velo Studio"</strong> to return.
          </p>
          <svg className="animate-spin w-6 h-6 text-[#5c5c5c] mx-auto mb-6" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
          {error && (
            <div className="p-2.5 rounded-xl bg-red-50 border border-red-100 text-red-600 text-xs mb-4">
              {error}
            </div>
          )}
          <button
            onClick={() => { setWaitingForOAuth(false); setError(''); }}
