...
function TimelineAxis({
  ...
  return (
    <div
      className="h-8 bg-white border-b border-neutral-200 relative overflow-hidden select-none"
      style={{
    ...
    >
      ...
      {markers.minorTicks.map((time) => {
        ...
        return (
          <div
            key={`minor-${time}`}
            className="absolute bottom-0 h-1 w-[1px] bg-neutral-200"
        ...
        return (
          <div key={marker.time} style={markerStyle}>
            <div className="flex flex-col items-center pb-1">
              <div className="h-2 w-[1px] bg-neutral-300 mb-1" />
              <span
                className={cn(
                  "text-[10px] font-medium tabular-nums tracking-tight",
                  marker.time === currentTimeMs ? "text-[#34B27B]" : "text-neutral-500"
                )}
              >
                {marker.label}
              </span>
...
function Timeline({
  ...
  return (
    <div
      ref={setRefs}
      style={style}
      className="select-none bg-neutral-50 min-h-[140px] relative cursor-pointer group"
      onClick={handleTimelineClick}
    >
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#00000005_1px,transparent_1px)] bg-[length:20px_100%] pointer-events-none" />
...
export default function TimelineEditor({
  ...
  if (!videoDuration || videoDuration === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center rounded-lg bg-neutral-50 gap-3">
        <div className="w-12 h-12 rounded-full bg-neutral-100 flex items-center justify-center">
          <Plus className="w-6 h-6 text-neutral-400" />
        </div>
        <div className="text-center">
          <p className="text-sm font-medium text-neutral-700">No Video Loaded</p>
          <p className="text-xs text-neutral-500 mt-1">Drag and drop a video to start editing</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-neutral-200 bg-white">
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            variant="ghost"
            size="icon"
            className="h-7 w-7 text-neutral-500 hover:text-[#ef4444] hover:bg-[#ef4444]/10 transition-all"
            title="Add Trim (T)"
          >
          ...
          <Button
            ...
            variant="ghost"
            size="icon"
            className="h-7 w-7 text-neutral-500 hover:text-[#B4A046] hover:bg-[#B4A046]/10 transition-all"
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="h-7 px-2 text-xs text-neutral-500 hover:text-neutral-700 hover:bg-neutral-100 transition-all gap-1"
              >
            ...
            <DropdownMenuContent align="end" className="bg-white border-neutral-200">
              {(['16:9', '9:16', '1:1', '4:3', '4:5'] as AspectRatio[]).map((ratio) => (
                <DropdownMenuItem
                  ...
                  className="text-neutral-700 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer flex items-center justify-between gap-3"
                >
        ...
        <div className="flex items-center gap-4 text-[10px] text-neutral-500 font-medium">
          <span className="flex items-center gap-1.5">
            <kbd className="px-1.5 py-0.5 bg-neutral-100 border border-neutral-200 rounded text-[#34B27B] font-sans">{shortcuts.pan}</kbd>
            <span>Pan</span>
          </span>
          <span className="flex items-center gap-1.5">
            <kbd className="px-1.5 py-0.5 bg-neutral-100 border border-neutral-200 rounded text-[#34B27B] font-sans">{shortcuts.zoom}</kbd>            
            <span>Zoom</span>
          </span>
        </div>
      </div>
